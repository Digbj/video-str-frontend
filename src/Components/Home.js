...
const HomePage = () => {
  ...
  useEffect(() => {
    const GetData = async () => {
      const url = `https://video-stream-backend-3ave.onrender.com/api/videos/get-random`;
      ...
      if (res.data) {
        const getUser = await axios.get(
          `https://video-stream-backend-3ave.onrender.com/api/users/get-User/${res.data[0].publisherId}`
        );
